...
export class ResizeHandle {  
    ...
    public getX = (): number => this.x + this.padding;
    public getY = (): number => this.y + this.padding;
    public getWidth = (): number => this.width - (this.padding * 2);
    public getHeight = (): number => this.height - (this.padding * 2);
    public getRotation = (): number => this.rotation;
    ...
    public setHandle = (option: IResizeHandle): void => {
        ...
        if (option.rotate !== undefined) this.rotation = option.rotate;
        this.notifyListeners();
    };
    ...
    private getHandleAtPosition(mouseX: number, mouseY: number): ResizeHandleSideOption | "MOVE" | null {
        ...
        for (const handle of handles) {
            if (canvasX >= handle.x && canvasX <= handle.x + hs &&
                canvasY >= handle.y && canvasY <= handle.y + hs) {
                return handle.id;
            }
        }
    ...
    private getCursorStyle(handle: ResizeHandleSideOption | "MOVE"): string {
        ...
        switch (handle) {
            ...
            case "TOP_LEFT":
            case "BOTTOM_RIGHT": return "nwse-resize";
            case "TOP_RIGHT":
            case "BOTTOM_LEFT": return "nesw-resize";
    ...
    private onMouseDown = (event: MouseEvent): void => {
        ...
        if (!handle || !this.isVisible) return;
    ...
    public getHandleArea = (): { x: number; y: number; width: number; height: number } => {
        return {
            x: this.x,
            y: this.y,
            width: this.width,
